...
public class DashboardService {
    ...
    public Map<String, Object> getDashboardData(LocalDateTime fromDate, LocalDateTime toDate) {
        ...
        if (userFromDate != null || userToDate != null) {
            ...
            Long resolvedComplaints = helpRequestRepository.countByStatusAndCreatedAtBetween(HelpRequestStatus.RESOLVED, userFromDate,
                    userToDate) +
                    reportRepository.countByStatusAndCreatedAtBetween(ReportStatus.RESOLVED, userFromDate, userToDate);
            payload.put("resolvedComplaints", resolvedComplaints);

            Long pendingComplaints = helpRequestRepository.countByStatusAndCreatedAtBetween(HelpRequestStatus.PENDING, userFromDate,
                    userToDate) +
    ...
    private List<Map<String, Object>> getPaymentTrend(LocalDateTime fromDate, LocalDateTime toDate) {
        ...
        for (int i = 2; i >= 0; i--) {
            ...
            if (!startDateTime.isAfter(endDateTime)) {
                totalAmount = razorpayPaymentRepository.sumSuccessfulPaymentsByDateRange(
                    PaymentStatus.CAPTURED, startDateTime, endDateTime);
            }
    ...
    private List<Profile> getProfilesByDateRange(LocalDateTime fromDate, LocalDateTime toDate) {
        if (fromDate != null && toDate != null) {
            Page<Profile> profilePage = profileRepository.findByCreatedAtBetween(fromDate, toDate, PageRequest.of(0, Integer.MAX_VALUE));
            return profilePage.getContent();
        } else if (fromDate != null) {
            Page<Profile> profilePage = profileRepository.findByCreatedAtGreaterThanEqual(fromDate, PageRequest.of(0, Integer.MAX_VALUE));
            return profilePage.getContent();
        } else if (toDate != null) {
            Page<Profile> profilePage = profileRepository.findByCreatedAtLessThanEqual(toDate, PageRequest.of(0, Integer.MAX_VALUE));
            return profilePage.getContent();
        }
    ...
    private List<Map<String, Object>> getRegionDistributionWithMoreStates(LocalDateTime fromDate, LocalDateTime toDate) {
        List<Map<String, Object>> regionDistribution = new ArrayList<>();
        ...
        for (Object[] result : results) {
            ...
            if (state != null) {
                ...
                } else if (state.equalsIgnoreCase("Delhi") || state.equalsIgnoreCase("NCR")) {
                    stateCounts.put("Delhi NCR", stateCounts.getOrDefault("Delhi NCR", 0) + count.intValue());
                } else {
                    othersCount += count.intValue();
                }
            }
